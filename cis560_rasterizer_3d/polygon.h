...
struct Vertex
{
    ...
    glm::vec4 c_pos;    // The position of the vertex in vcs;
    glm::vec4 w_pos;    // The position of the vertex in wcs;

    Vertex(glm::vec4 p, glm::vec3 c, glm::vec4 n, glm::vec2 u)
        : m_pos(p), m_color(c), m_normal(n), m_uv(u), w_pos(p)
    {}
};
...
struct Triangle
{
    ...
    // Store bounding box of this triangle
    float x_min = 100000.f;
    float y_min = 100000.f;
    float x_max = 0.f;
    float y_max = 0.f;
};
...
public:
    ...
    std::vector<float> endpoint1;
    std::vector<float> endpoint2;
    // A variable for storing the slope of the line segment
    float slope;

    // Default constructor
    Segment();
    // Constructor takes in both endpoints of line segment
    Segment(const std::vector<float> &e1, const std::vector<float> &e2);
    // Conputes x-intersection of the line segment with a horizontal line
